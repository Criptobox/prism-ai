...
| 📏 **El agente mide su propio cambio (`run_regression`)** | Ejecuta el proyecto y lo compara con su ejecución anterior: qué errores son nuevos, cuáles arregló, qué pasó con el QA móvil y con el peso. La primera vez no hay con qué comparar, y lo dice en vez de inventarse un antes. Dos páginas distintas no se comparan. |
| 🔍 **Qué se movió (`snapshot_diff`)** | Compara un punto de restauración con el proyecto actual, o dos entre sí: archivos modificados, nuevos y borrados con sus líneas. Aquí no hay git ni ramas: son los snapshots que ya guardaba `git_snapshot`, en tu dispositivo. |
...
| 🚫 **No tocar: archivos que el agente no puede escribir** | Marca `Header.tsx` o `src/api/*` en el mapa del proyecto y el agente deja de poder escribirlos: la regla viaja en el prompt **y** se rechaza en el ejecutor si aun así lo intenta. A diferencia de una nota, esto se hace cumplir. Restaurar un punto de guardado que cambiaría un archivo protegido se cancela entero, no a medias. Tú sí puedes editarlo a mano. |
| 🔎 **Se ve qué contexto viajó** | Bajo cada respuesta: `ctx 2 archivos · 2 notas · 1 regla · 8 mensajes`. Un clic y salen los nombres. Se cuenta lo que ENTRÓ en el prompt, no lo que hay guardado — y si no había nada del proyecto, el chip no sale: un aviso que aparece siempre deja de leerse. |
| 🎯 **Dirigir un equipo (`/orquesta`)** | Tu modelo —el mejor que tengas— parte el encargo, varios modelos gratis hacen cada trozo en paralelo, y el tuyo revisa y cierra. Coste acotado por diseño: `2 + n` llamadas y **se dice antes de arrancar**. Cada ejecutor ve solo su parte, no tu conversación. Sin inventar precios: se cuentan llamadas, no dólares. |
| 💸 **Techo de llamadas de pago al día** | Encendido de fábrica en 200. Solo cuenta los modelos que no son gratis, se reinicia cada día y corta **antes** de que la petición salga del navegador —chat, failover, panel y ejecutores del orquestador incluidos—. No convierte llamadas en dinero: los precios cambian por proveedor y por modelo, y un «≈ 2,40 $» inventado sería peor que un número honesto de llamadas. |
| 🚫 **Vetar un proveedor sin borrar su clave** | Un botón por proveedor en Ajustes: queda conectado pero **no recibe ni una petición**, ni siquiera por los caminos que eligen solos (failover, panel de consenso, ejecutores). Borrar la clave también valía, pero te dejaba sin ese proveedor para todo. El veto vale hacia delante: lo que ya le mandaste, mandado está. |
| 📊 **En qué se te va: modelo × encargo** | El Panel del sistema abre en «Gasto»: qué modelo de pago se llevó cada tipo de encargo —página web, código, escritura, datos, razonamiento, chat—, con llamadas, caracteres y tokens **aproximados** (car. ÷ 4, siempre con «≈»). Ni un euro en pantalla: los precios cambian por modelo y por tramo, y un importe inventado se leería como un dato. Lo registrado antes de la v3.48 se cuenta como «sin clasificar» en vez de repartirse a ojo. |
